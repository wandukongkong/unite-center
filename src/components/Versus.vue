		...
		<div class="d-flex my-2 team-area position-relative">
			...
			<transition-group>
				<PokemonCard v-for="(aPlayer, index) in aTeams" :key="index" :player="aPlayer"
					:pokemonInfo="aTeamPokemonList[index]" :isOpen="props.isAllOpen"></PokemonCard>
			</transition-group>
		</div>
		<Transition>
			<div class=" d-flex flex-fill w-100 h-100 justify-content-center align-items-center position-absolute z-1"
		...
		<div class="d-flex my-2 team-area position-relative" v-if="_.size(bTeams) > 0">
			...
			<transition-group>
				<PokemonCard v-for="(bPlayer, index) in bTeams" :key="index" :player="bPlayer"
					:pokemonInfo="bTeamPokemonList[index]" :isOpen="props.isAllOpen"></PokemonCard>
			</transition-group>
		</div>
	</div>
</template>
